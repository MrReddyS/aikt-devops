...
require_resource_group() {
  local rg="$1"
  if ! run_az group show --name "${rg}" >/dev/null 2>&1; then
    echo "ERROR: resource group \"${rg}\" does not exist. Run Resource_Group/create_rg.sh first." >&2
    return 1
  fi
}

# Strip CR from config values (Windows CRLF line endings in config.yaml).
strip_cr() {
  local v="${1:-}"
  v="${v//$'\r'/}"
  printf '%s' "${v}"
...
# Windows Python (miniconda/py.exe) cannot open Git Bash /c/Users/... paths reliably.
python_path() {
  local p
  p="$(strip_cr "${1:-}")"
  ...
  if [[ "${p}" =~ ^/([a-zA-Z])/(.*)$ ]]; then
    printf '%s:/%s' "$(echo "${BASH_REMATCH[1]}" | tr '[:lower:]' '[:upper:]')" "${BASH_REMATCH[2]}"
    return 0
  fi
  printf '%s' "${p}"
}
...
# Login, set subscription (name or GUID), then re-set using resolved GUID.
ensure_az_account() {
  local tenant_id="$1"
  ...
  resolved="$(run_az account show --query id -o tsv 2>/dev/null || true)"
  resolved="${resolved//$'\r'/}"
  if [[ -z "${resolved}" || "${resolved}" == "null" ]]; then
    echo "ERROR: could not resolve subscription \"${subscription}\"." >&2
    return 1
  fi
  run_az account set --subscription "${resolved}" >/dev/null
}
...
resolve_profile_name() {
  ...
  if [[ -n "${explicit}" ]]; then
    if command -v yq >/dev/null 2>&1; then
      ...
      "${DEVOPS_PY[@]}" - "$(python_path "${cfg}")" "${explicit}" <<'PY' >/dev/null || return 1
import sys
from pathlib import Path
try:
    import yaml
...
read_subscription_profile() {
  ...
  if command -v yq >/dev/null 2>&1; then
    ...
    read -r TENANT_ID SUBSCRIPTION_ID DEFAULT_LOCATION < <("${DEVOPS_PY[@]}" - "$(python_path "${cfg}")" "${profile}" <<'PY'
import sys
from pathlib import Path
cfg_path = Path(sys.argv[1])
profile = sys.argv[2]
